//pi control, lcd with buttons, need to change gain for the ambient
...
if(select >=2){//condition for when select is bigger than the number
  ...
  select = 2;//= the number if it passes it
}
...
if(b1.isPressed())//library command for when b1 is pressed
  gain = gain +0.05;//hys+15
    if (gain >= 5.0)//hys capped at 155
   gain = 5.0;
...
if(b1.isPressed())//library command for when b1 is pressed
  inter = inter +0.2;//sp + 100
    if (inter >= 10)//sp capped at 1030
   inter = 10;

if(b2.isPressed())//library command for when b2 is pressed
  inter = inter -0.2;//sp -100
   if (inter <= 0)//sp capped at 0
   inter = 0;



lcd.setCursor(0, 1);//setup the position for the text x=0 y=1
lcd.print("Ki=  ");//print the text
lcd.print(inter);//print the value of sp
lcd.print("           ");//print spaces to avoid visual bugs
}
...
interact = (interact + error);
      CO = ((gain*error)+(gain*interact))/100;
...
Serial.print("    Kp=  ");//prints value of hys
Serial.print(gain);
Serial.print("       ");

Serial.print("    Ki=  ");//prints value of hys
Serial.print(inter);
Serial.print("       ");
*/
Serial.print("    error=  ");//prints value of hys
Serial.print(error);
Serial.print("       ");
